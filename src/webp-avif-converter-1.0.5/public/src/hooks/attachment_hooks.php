...
namespace WebpAvifConverter\Hooks;

use WebpAvifConverter\Utils;
use WebpAvifConverter\GeneratorUtils;
...
add_filter('wp_generate_attachment_metadata', 'WebpAvifConverter\Hooks\convert_images_on_generate_attachment_metadata', 10, 2);
...
add_action('edit_attachment', 'WebpAvifConverter\Hooks\save_image_quality_metadata');
function save_image_quality_metadata($attachment_id)
{
	...
	foreach ($filed_names as $filed_name) {
		if (empty($_REQUEST['attachments'][$attachment_id][$filed_name])) {
			continue;
		}

		$quality = intval($_REQUEST['attachments'][$attachment_id][$filed_name]);

		if ($quality < 1 || $quality > 100 || isset($metadata[$filed_name]) && $quality === $metadata[$filed_name]) {
			continue;
		}

		if ($filed_name === 'quality_jpeg') {
			GeneratorUtils\generate_jpeg_sizes($attachment_id, $quality);
		}
		if ($filed_name === 'quality_webp') {
			GeneratorUtils\generate_webp_sizes($attachment_id, $quality);
		}
		if ($filed_name === 'quality_avif') {
			GeneratorUtils\generate_avif_sizes($attachment_id, $quality);
		}
	}
}
...
add_filter('delete_attachment', 'WebpAvifConverter\Hooks\delete_attachment_files');
